...
public class DoiHang_GUI extends JPanel {
    ...
    private JPanel createRightPanel() {
        ...
        for (LoaiSanPham l : categories) {
            cbCategories.addItem(l.getTenLoaiSP());
        }
        ...
        modelInventory = new DefaultTableModel(invCols, 0) {
            public boolean isCellEditable(int r, int c) {
                return false;
            }
        };
    ...
    private void refreshOldOrderTable() {
        ...
        oldOrderTotal = 0;
        for (ReturnItem i : currentInvoiceItems) {
            double total = i.returnQty * i.price;
            oldOrderTotal += total;
            ...
            if (i.returnQty > 0) {
                if (i.reason == null || i.reason.isEmpty()) {
                    i.reason = "Sản phẩm lỗi";
                }
    ...
    private void selectReturnQuantity() {
        int r = tblOldOrder.getSelectedRow();
        if (r < 0) {
            return;
        }
